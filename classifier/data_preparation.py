...
from utils import * # cocoaaa: my utils functions
...
def data_preparation(image_dir: Union[Path,str], 
                     tfrecord_dir, 
                     resolution=256, 
                     max_samples_per_subdir: Optional[int]=100_000,
                     shuffle=1, 
                     export_labels=1, 
    ...
    sources_dir = sorted([p for p in image_dir.iterdir() if is_valid_dir(p)])
    source_c2i = {source_dir.name:np.uint32(i) for (i,source_dir) in enumerate(sources_dir)}

    image_filenames = []
    for source_dir in sources_dir:
        # image_filenames_temp = sorted(glob.glob(os.path.join(image_dir, source_dir, '*.png')))
        image_filenames_temp = sorted([str(fp) for fp in source_dir.iterdir() if is_img_fp(fp)])
        
        # n_samples = int(float(len(image_filenames_temp))*float(percentage_samples)/100.0)
        image_filenames += image_filenames_temp[:max_samples_per_subdir]
    print('Loaded total images: ', len(image_filenames) )
    if len(image_filenames) == 0:
        error('No input images found')
        
    img = np.array(pil_rgb_loader(image_filenames[0])) #note: currently only considers rgb images (not grayscale)
    size = img.shape[0]
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle==1 else np.arange(len(image_filenames))
        for idx in range(order.size):
            ...
            img = pil_rgb_loader(image_filenames[order[idx]])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resolution', type=int, default=256) # The resolution to which images are resized
    ...
    parser.add_argument('--max_samples_per_subdir', type=int, default=100_000) # Export image source labels?

    args = parser.parse_args()
    data_preparation(
        image_dir=args.in_dir, 
        tfrecord_dir=args.out_dir, 
        resolution=args.resolution, 
        max_samples_per_subdir=args.max_samples_per_subdir,
        shuffle=args.shuffle, 
        export_labels=args.export_labels, 
        # args.percentage_samples
        )